...
def bollywood():
    ...
    #creating window
    bolly=Tk()
    bolly.title('Bollywood')
    bolly.configure(bg='LightBlue2')
    ...
    def f7():
       global b,Year,Actor,Actress
       my=c.connect(host="localhost",user="root",passwd="1234",database="movie_mania")
       choose=my.cursor()
       choose.execute("select * from m_movie where Type='Bollywood' order by rand()limit 1;")
       p=choose.fetchall()
       for i in p:
        l=list(i)
        b=l[0]
    ...
    def create_question():
            ...
            s=''.join(ques)
            Label(bolly,text=s,font=('Times New Roman', 30),fg='white',bg='LightBlue2').grid(row=0,column=0,columnspan=13,pady=20)
    create_question()   
    ...
    for j,k in alp:
         ...
         Button(bolly,text=j,command=lambda j=j:guess(j),font=('Times New Roman',18),width=4,bg=k,fg='White',borderwidth=10).grid(row=1+n//13,column=n%13)
         n+=1
    ...
    global chance
    chance=s.count('*')
    status=Label(bolly,text='Chances Remaining: '+str(chance),font=('Times New Roman',18),bd=1,relief='sunken',anchor=E)
    ...
    def guess(j):    # checking if the guessed letter is in movie or not
        ...
        s=''
        if (j not in already):
             if j in b:       #if guessed letter is in the movie
                    for i in range(len(b)):
                              if (b[i]==j):
                    ...
                    if (s!=b):      #if movie is not guessed completely   
                       Label(bolly,text=s,font=('Times New Roman', 30),bg='LightBlue2',fg='White').grid(row=0,column=0,columnspan=13,pady=20)
                       
                    
                    
                    elif(s==b):      #if movie is guessed completely
                        Label(bolly,text=s,font=('Times New Roman', 30),bg='LightBlue2',fg='White').grid(row=0,column=0,columnspan=13,pady=20)
                        final=points+chance
                        ...
                        check=Button(bolly,text='Check Leaderboard',command=lambda:[leaderboard(),bolly.destroy()],font=('Times New Roman', 23),borderwidth=10,bg='brown',fg='white')
                        check.grid(row=7,column=0,columnspan=13)            
                      
                        
             
             elif(j not in b):     #if guessed letter is not in the movie
    ...
    f=1
    bend=Button(bolly,text='END',command=lambda:end(),font=('Times New Roman', 17),borderwidth=10,bg='Red',fg='White')
    bend.grid(row=6,column=12,pady=20)
    bhint=Button(bolly,text='HINT',command=lambda:hint(f),font=('Times New Roman', 16),borderwidth=10,bg='green2',fg='White')
    ...
    def hint(f):
            ...
            if(f==1):
                ...
                f+=1
                bhint=Button(bolly,text='HINT',command=lambda:hint(f),font=('Times New Roman', 16),borderwidth=10,bg='green2',fg='White').grid(row=6,column=0,pady=20)
            elif(f==2 ):
